...
const salt='0x00000000000000000000000000000000000000000000000000000000abcd0001'
const data=iface.encodeFunctionData('createLaunch',[cfg,NFTF,vaultData,salt])
try {
  const g = await p.estimateGas({to:LF,from:ME,value:ethers.parseEther('0.005'),data})
  console.log('createLaunch 实际需要 gas:', g.toString())
} catch(e){ console.log('estimateGas 失败:', e.reason||e.shortMessage||e.message) }
